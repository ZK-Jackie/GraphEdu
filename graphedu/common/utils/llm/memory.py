...
def _filter_keys(keys: list[str], before: RunnableConfig | None, limit: int | None) -> list:
    """Filter and sort Redis keys based on optional criteria."""
    if before:
        keys = [
            k
            for k in keys
            if _parse_redis_checkpoint_key(k.decode())["checkpoint_id"] < before["configurable"]["checkpoint_id"]
        ]
        ...
        *,
        ...
        for key in keys:
            ...
            if data and b"checkpoint" in data and b"metadata" in data:
                ...
                pending_writes = self._load_pending_writes(thread_id, checkpoint_ns, checkpoint_id)
                yield _parse_redis_checkpoint_data(self.serde, key.decode(), data, pending_writes=pending_writes)
    ...
    def _load_pending_writes(
        ...
        return _load_writes(
            self.serde,
            {
                ...
                for key, parsed_key in sorted(zip(matching_keys, parsed_keys, strict=False), key=lambda x: x[1]["idx"])
            },
        )

    def _get_checkpoint_key(self, conn, thread_id: str, checkpoint_ns: str, checkpoint_id: str | None) -> str | None:
...
class AsyncRedisSaver(BaseCheckpointSaver):
    ...
    async def from_conn_info(cls, *, host: str, port: int, db: int) -> AsyncIterator["AsyncRedisSaver"]:
        """从连接信息创建 AsyncRedisSaver 实例的异步上下文管理器。
        ...
        Yields:
            AsyncRedisSaver: 异步 Redis 检查点保存器实例
        """
        conn = None
        try:
    ...
    async def _aget_checkpoint_key(
        self, conn, thread_id: str, checkpoint_ns: str, checkpoint_id: str | None
    ) -> str | None:
