<template>
  <a-radio-group v-bind="$attrs" v-model:value="innerValue" :loading="loading" @change="handleChange">
    <a-radio v-for="item in dictOptions" :key="item.value" :value="item.value" :disabled="item.disabled">
      <span v-if="!renderStyle" class="dict-radio-text">
        <SvgIcon v-if="item.icon" :icon="item.icon" class="dict-radio-icon" />
        {{ item.label }}
      </span>
